<?php
/**
 * MedWell Pharmacy - Add Medicine Page
 *
 * Premium modal-style form page for adding new medicines
 * with validation, icons, and CSRF protection.
 * PHP 8.0+
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        redirect('/modules/medicines/add.php');
    }
    ...
    // Insert if no errors
    if (empty($errors)) {
        $newId = $medicineObj->create($formData);
        if ($newId) {
            // Log inventory if initial quantity > 0
            if ($formData['quantity'] > 0) {
                logInventory($newId, 'in', 0, $formData['quantity'], 'initial_stock', null, 'Initial stock on medicine creation');
            }

            regenerateCsrfToken();
            flashMessage('medicine_success', "Medicine \"{$formData['name']}\" has been added successfully.", 'success');
            redirect('/modules/medicines/');
        } else {
            $errors['general'] = 'Failed to add medicine. Please try again.';
        }
    }
}
?>
    ...
    <main class="main-content">
        ...
        <div class="form-page-header">
            <div class="header-left">
                <a href="/modules/medicines/" class="back-btn" title="Back to Medicines">
                    <i class="ri-arrow-left-line"></i>
                </a>
                <div>
                    <h2>Add Medicine</h2>
                    <div class="subtitle">Add a new medicine to your inventory</div>
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <div>
                    ...
                    <p>Fill in the information below to add a new medicine</p>
                </div>
            </div>

            <form method="POST" action="" id="addMedicineForm" novalidate>
                ...
                <div class="form-section">
                    <div class="form-section-title">
                        <i class="ri-barcode-line"></i> Batch & Identification
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrapper">
                                <select name="supplier_id">
                                    ...
                                    <option value="<?= $sup['id'] ?>" <?= ($formData['supplier_id'] ?? '') == $sup['id'] ? 'selected' : '' ?>>
                                        <?= sanitize($sup['name']) ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                ...
                <div class="form-section">
                    <div class="form-section-title">
                        <i class="ri-money-dollar-circle-line"></i> Pricing & Stock
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
                            <label>Selling Price <span class="required">*</span></label>
                            <div class="input-icon-wrapper">
                                <input type="number" name="price" value="<?= $formData['price'] ?? '' ?>" placeholder="0.00" step="0.01" min="0" required class="<?= !empty($errors['price']) ? 'is-invalid' : '' ?>">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrapper">
                                <input type="number" name="cost_price" value="<?= $formData['cost_price'] ?? '' ?>" placeholder="0.00" step="0.01" min="0" class="<?= !empty($errors['cost_price']) ? 'is-invalid' : '' ?>">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrapper">
                                <input type="number" name="quantity" value="<?= $formData['quantity'] ?? 0 ?>" placeholder="0" min="0">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrapper">
                                <input type="number" name="min_stock_level" value="<?= $formData['min_stock_level'] ?? 10 ?>" placeholder="10" min="0">
                ...
                <div class="form-section">
                    <div class="form-section-title">
                        <i class="ri-calendar-line"></i> Manufacture & Expiry Dates
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
                ...
                <div class="form-card-footer">
                    ...
                    <button type="submit" class="btn btn-primary btn-sm">
                        <i class="ri-check-line"></i> Save Medicine
                    </button>
                </div>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (batchInput && !batchInput.value) {
        const now = new Date();
        const batch = 'BN-' + now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(Math.floor(Math.random() * 9999)).padStart(4, '0');
