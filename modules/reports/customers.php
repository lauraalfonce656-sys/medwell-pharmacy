...
$customerData = $report->getCustomerReport();
$topCustomers = $report->getTopCustomers(10);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Customer Report - MedWell Pharmacy</title>
    ...
    <style>
        ...
        .badge-purchase { background: #E8F5E9; color: #2E7D32; padding: 5px 12px; border-radius: 8px; font-weight: 600; }
        .badge-points { background: #F3E5F5; color: #7B1FA2; padding: 5px 12px; border-radius: 8px; font-weight: 600; }
        .customer-avatar {
            width: 36px; height: 36px; border-radius: 10px; display: inline-flex;
            align-items: center; justify-content: center; font-weight: 700;
            font-size: 0.85rem; color: #fff; margin-right: 10px;
        }
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar">
            <h4><i class="ri-user-line" style="color:#9C27B0"></i> Customer Report</h4>
        ...
        <div class="content-area">
            ...
            <div class="chart-card">
                <h5><i class="ri-bar-chart-horizontal-line" style="color:#9C27B0"></i> Top 10 Customers by Purchase Amount</h5>
                <canvas id="customerChart" height="80"></canvas>
            </div>

            <!-- Data Table -->
            <div class="data-card">
                <h5><i class="ri-table-line" style="color:var(--mw-green)"></i> Customer Data</h5>
                    ...
                    <table id="customerTable" class="table table-hover" style="width:100%">
                        ...
                        <tbody>
                            <?php if(!empty($customerData)): ?>
                                <?php
                                $avatarColors = ['#7CB342','#2196F3','#FF9800','#9C27B0','#009688','#F44336','#3F51B5','#E91E63','#00BCD4','#FF5722'];
                                $ci = 0;
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <span class="customer-avatar" style="background:<?php echo $color; ?>"><?php echo $initials; ?></span>
                                            <span class="fw-semibold"><?php echo htmlspecialchars($row['customer_name']); ?></span>
                                        </div>
                                    </td>
                                    <td><span class="badge-purchase">$<?php echo number_format($row['total_purchases'], 2); ?></span></td>
                                    <td><?php echo number_format($row['num_orders']); ?></td>
                                    <td>$<?php echo number_format($row['avg_order'], 2); ?></td>
                                    <td><?php echo date('M d, Y', strtotime($row['last_purchase'])); ?></td>
                                    <td><span class="badge-points"><?php echo number_format($row['loyalty_points']); ?> pts</span></td>
                                </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
    ...
    <script>
        ...
        const barColors = ['#7CB342','#8BC34A','#9CCC65','#2196F3','#42A5F5','#64B5F6','#9C27B0','#AB47BC','#BA68C8','#CE93D8'];

        new Chart(document.getElementById('customerChart').getContext('2d'), {
            type: 'bar',
            data: {
                labels: topCustomers.map(d => d.customer_name),
                datasets: [{
                    label: 'Total Purchases ($)',
                    data: topCustomers.map(d => parseFloat(d.total_purchases)),
        ...
        function exportCSV() {
            ...
            a.href = url; a.download = 'customer_report_<?php echo date('Y-m-d'); ?>.csv';
